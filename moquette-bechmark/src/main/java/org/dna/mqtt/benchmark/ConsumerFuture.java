...
public class ConsumerFuture implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < Producer.PUB_LOOP; i++) {
			...
			StringBuffer sb = new StringBuffer().append("Topic: ").append(message.getTopic()).append(", payload: ")
					.append(new String(payload));
			LOG.debug(sb.toString());
		}

		long time = System.currentTimeMillis() - startMillis;
		LOG.info(String.format("Consumer %s received %d messages in %d ms", Thread.currentThread().getName(),
				Producer.PUB_LOOP, time));

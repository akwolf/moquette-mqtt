...
public final class Client {
	...
	public void publish(String topic, byte[] payload, AbstractMessage.QOSType qos, boolean retain)
	...
	private void manageSendQoS1(MessageIDMessage msg) throws Throwable {
		...
		//if not arrive into certain limit, raise an error
		if (!unlocked) {
			throw new SubscribeException(String.format("Server doesn't replyed with a SUB_ACK after %d replies",
					RETRIES_QOS_GT0));
		} else {
			...
			if (m_receivedSubAckMessageID != messageID) {
				throw new SubscribeException(String.format("Server replyed with "
						+ "a broken MessageID in SUB_ACK, expected %d but received %d", messageID,
						m_receivedSubAckMessageID));
			}
		}
	}
	...
	private void updatePinger() {
		...
		m_pingerHandler = m_scheduler.scheduleWithFixedDelay(pingerDeamon, KEEPALIVE_SECS, KEEPALIVE_SECS,
				TimeUnit.SECONDS);
	}
